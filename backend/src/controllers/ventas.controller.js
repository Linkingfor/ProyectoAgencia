/* ════════════════════════════════════════════════════
   Ventas — Registro de cobros + Comprobante PDF (RF-07)
   ════════════════════════════════════════════════════
   Una "venta" representa el cobro hecho a un cliente. Cuando se
   crea, también se genera automáticamente el comprobante (boleta o
   factura) con un número correlativo (B001-000001, F001-000001...).

   El PDF se genera "on demand" con pdfkit cuando alguien pide
   /api/ventas/:id/pdf. Lo pueden descargar:
     - Cualquier trabajador de la intranet.
     - SOLO el cliente dueño de la venta (revisado en downloadPdf).
   ════════════════════════════════════════════════════ */
...
const downloadPdf = async (req, res) => {
  try {
    ...
    doc.fillColor('#ffffff').fontSize(22).font('Helvetica-Bold').text('AGENCIA DE VIAJES ICA', 60, 65)
      .fontSize(10).font('Helvetica').text('Sistema de Gestión Turística - Proyeco agencia.', 60, 95)
