/* ════════════════════════════════════════════════════
   Reservas — Corazón del control de cupos (RF-04)
   ════════════════════════════════════════════════════
   Aquí están las 4 validaciones que evitan que la agencia
   sobrevenda tours o asigne pasajeros a vehículos equivocados:

     1. La cantidad de personas no puede pasar la capacidad del SERVICIO.
     2. La suma de pax de ese día/servicio no puede pasar la capacidad.
     3. Si la reserva se ata a una SALIDA, la fecha debe coincidir.
     4. Si se ata a una salida, los pax + los ya reservados no pueden
        pasar la capacidad del VEHÍCULO de esa salida.

   Todo dentro de una transacción (BEGIN/COMMIT/ROLLBACK) para que
   dos personas reservando a la vez no dejen el sistema inconsistente.
   ════════════════════════════════════════════════════ */
...
const create = async (req, res) => {
  const { id_cliente, id_servicio, fecha_servicio, cantidad_personas, id_salida, origen_reserva, observaciones } = req.body;
  ...
  try {
    ...
    const ins = await client.query(
      ...
      [fecha_servicio, cant, origen_reserva || 'intranet', observaciones || null, id_cliente, id_servicio, id_salida || null]
    );
...
// PATCH /api/reservas/:id/observaciones  → guardar/editar las notas (RFC-005)
...
module.exports = { getAll, getById, checkDisponibilidad, create, updateEstado, updateObservaciones, asignarSalida, remove };
